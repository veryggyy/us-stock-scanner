...
if st.button("🚀 開始多頭掃描 (排序由強至弱)", use_container_width=True):
    if df_all is not None:
        ...
        if results:
            ...
            st.download_button("📥 匯出清單 (CSV)", res_df.to_csv(index=False).encode('utf-8-sig'), "stocks.csv", "text/csv")
...
st.divider()
st.caption("⚠ 免責聲明：此程式僅供參考。美股無漲跌幅限制，請務必設置止損位。")
